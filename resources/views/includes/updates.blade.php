    ...
    <div class="card mb-3 card-updates" data="{{$response->id}}">
                                    ...
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h6 class="modal-title font-weight-light" id="modal-title-default"><i
                                                    class="fas fa-flag mr-1"></i> {{trans('admin.report_update')}}</h6>
                                        ...
                                        <form method="POST" action="{{url('report/update', $response->id)}}"
                                            ...
                                            <!-- Start Form Group -->
                                                <div class="form-group">
                                                    ...
                                                    <select name="reason" class="form-control custom-select">
                                                        ...
                                                        <option
                                                            value="violent_sexual">{{trans('admin.violent_sexual_content')}}</option>
                                                    </select>
                                                </div><!-- /.form-group-->
                                            </div><!-- Modal body -->
                                            ...
                                            <div class="modal-footer">
                                                <button type="submit" class="btn btn-xs btn-white sendReport">
                                                    <i></i> {{trans('admin.report_update')}}</button>
            ...
            <div class="btn-block">
                ...
                @if($response->image != '')
                    ...
                    <a href="{{ $urlImg }}" data-group="gallery{{$response->id}}" class="js-smartPhoto w-100">
                        <img src="{{$urlImg}}?w=130&h=100" data-src="{{$urlImg}}?w=960&h=980"
                ...
                @if ($response->video_embed != '' && in_array(Helper::videoUrl($response->video_embed), array('youtube.com','www.youtube.com','youtu.be','www.youtu.be')))
                    <div class="embed-responsive embed-responsive-16by9 mb-2">
                        <iframe class="embed-responsive-item" height="360"
                                src="https://www.youtube.com/embed/{{ Helper::getYoutubeId($response->video_embed) }}"
                ...
                @if ($response->video_embed != '' && in_array(Helper::videoUrl($response->video_embed), array('vimeo.com','player.vimeo.com')))
                    <div class="embed-responsive embed-responsive-16by9">
                        <iframe class="embed-responsive-item"
                                src="https://player.vimeo.com/video/{{ Helper::getVimeoId($response->video_embed) }}"
                                allowfullscreen></iframe>
                    </div>
                @endif
        ...
        @else
            ...
            <div class="btn-block p-sm text-center content-locked pt-lg pb-lg px-3">
                ...
                @elseif ($response->user()->price != 0.00 && $response->price != 0.00
                        || $response->user()->free_subscription == 'yes' && $response->price != 0.00)
                    <a href="javascript:void(0);" @guest data-toggle="modal" data-target="#loginFormModal"
                       @else data-toggle="modal" data-target="#payPerViewForm" data-mediaid="{{$response->id}}"
                       data-price="{{Helper::amountFormatDecimal($response->price)}}"
                       data-pricegross="{{$response->price}}" @endguest class="btn btn-primary w-100">
                        @guest
                            {{ trans('general.content_locked_user_logged') }}
                        @else
                ...
                @if ($response->image != '')
                    <h6 class="btn-block mt-2 font-weight-light"><i
                            class="feather icon-image"></i> {{ __('general.photo') }}</h6>
                @endif

                @if ($response->video != '' || $response->video_embed)
                    <h6 class="btn-block mt-2 font-weight-light"><i
                            class="feather icon-video"></i> {{ __('general.video') }}</h6>
                @endif

                @if ($response->music != '')
                    <h6 class="btn-block mt-2 font-weight-light"><i
                            class="feather icon-mic"></i> {{ __('general.audio') }}</h6>
                @endif

                @if ($response->file != '')
                    <h6 class="btn-block mt-2 font-weight-light"><i
                            class="far fa-file-archive"></i> {{ __('general.file') }} - {{ $response->file_size }}</h6>
                @endif
                ...
                @if ($response->image == ''
                    ...
                    <h6 class="btn-block mt-2 font-weight-light"><i
                            class="feather icon-file-text"></i> {{ __('admin.text') }}</h6>
                @endif
        ...
        <div class="card-footer bg-white border-top-0">
            <h4>
                ...
                <a href="javascript:void(0);" @guest data-toggle="modal" data-target="#loginFormModal"
                   @endguest class="btnLike @if($likeActive)active @endif {{$buttonLike}} text-muted mr-2"
                ...
                {{--            <button style="border: none!important;" class=" text-muted btn btn-profile btn-google" title="{{trans('general.share')}}" id="dropdownUserShare" role="button" data-toggle="modal" data-target=".share-modal">--}}
                ...
                <a href="javascript:void(0);" @guest data-toggle="modal" data-target="#loginFormModal"
                   @endguest class="@if($bookmarkActive) text-primary @else text-muted @endif float-right {{$buttonBookmark}}"
                   @auth data-id="{{$response->id}}" @endauth>
