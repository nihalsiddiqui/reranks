		...
		<div class="container-fluid d-flex">
			<a class="navbar-brand margin-auto" href="{{url('/')}}">
				@if (auth()->check() && auth()->user()->dark_mode == 'on' )
					<img src="{{url('public/img', $settings->logo)}}" data-logo="{{$settings->logo}}" data-logo-2="{{$settings->logo_2}}" alt="{{$settings->title}}" class="logo align-bottom" style="width:50px!important" />
				@else
				<img src="{{url('public/img', auth()->guest() && request()->path() == '/' ? $settings->logo : $settings->logo_2)}}" data-logo="{{$settings->logo}}" data-logo-2="{{$settings->logo_2}}" alt="{{$settings->title}}" class="logo align-bottom" style="width:50px!important" />
			@endif
			</a>
				...
				<!-- =========== End Menu Mobile -->
                    ...
                    <li class="nav-item dropdown d-lg-block d-none">
						<a class="nav-link px-2"  href="{{route('group.create')}}" title="{{trans('admin.group')}}">
							<i class="feather icon-users icon-navbar"></i>
							<span class="d-lg-none align-middle ml-1">{{trans('admin.group')}}</span>
						</a>
					</li>
                    @endif
